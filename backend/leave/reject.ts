...
export const reject = api<ApproveLeaveRequest, LeaveRequest>(
  { expose: true, method: "POST", path: "/leave/requests/:leave_request_id/reject" },
  async (req) => {
    ...
    if (!leaveRequest) {
      throw APIError.internal("Không thể từ chối đơn xin nghỉ");
    }
