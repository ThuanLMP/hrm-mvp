...
import db from "../db";
import { MonthlyPayrollRequest, MonthlyPayrollResponse, PayrollRecord, PayrollSummary } from "./types";

// Calculate monthly payroll for all active employees
export const calculateMonthly = api<MonthlyPayrollRequest, MonthlyPayrollResponse>(
  { expose: true, method: "POST", path: "/payroll/calculate" },
  async (req) => {
    ...
    const employeesQuery = `
      ...
      WHERE e.status = 'active' AND e.salary IS NOT NULL ${employeeFilter}
    ...
    for (const employee of employees) {
      ...
      const attendanceQuery = `
        ...
        WHERE t.employee_id = $1 
          AND EXTRACT(MONTH FROM t.work_date) = $2 
          AND EXTRACT(YEAR FROM t.work_date) = $3
      `;

      const attendanceResult = await db.rawQueryRow<{
        total_records: number;
        work_days: number;
        total_overtime_text: string;
      }>(attendanceQuery, employee.id, month, year);
      ...
      const statusResults = await db.rawQueryAll<{
        work_date: Date;
        check_in?: Date;
        check_out?: Date;
        checkin_status: string;
        checkout_status: string;
      }>(statusQuery, employee.id, month, year);
      ...
      const lateCount = statusResults.filter(r => r.checkin_status === 'late').length;
      const earlyLeaveCount = statusResults.filter(r => r.checkout_status === 'early_leave').length;
      ...
      const baseSalaryAmount = dailySalary * Math.min(attendance.work_days, workingDaysInMonth);
      
      const overtimeAmount = (salary / settings.working_days_per_month / 8) * 
                            totalOvertimeHours * settings.overtime_rate;
      
      const latePenalty = lateCount * settings.late_penalty_per_day;
      const earlyLeavePenalty = earlyLeaveCount * settings.early_leave_penalty_per_day;
    ...
    const summary: PayrollSummary = {
      ...
      total_gross_salary: payrollRecords.reduce((sum, record) => sum + record.gross_salary, 0),
      total_net_salary: payrollRecords.reduce((sum, record) => sum + record.net_salary, 0),
      total_overtime_amount: payrollRecords.reduce((sum, record) => sum + record.overtime_amount, 0),
      total_bonus_amount: payrollRecords.reduce((sum, record) => sum + record.bonus_amount, 0),
      total_deduction_amount: payrollRecords.reduce((sum, record) => sum + record.deduction_amount, 0),
    };
