...
import { Plus, Calendar, Check, X, User, FileText, Clock, BarChart3 } from 'lucide-react';
...
export function LeavePage() {
  ...
  const { data: leaveBalance, isLoading: balanceLoading, error: balanceError } = useQuery({
  ...
  const { data: leaveRequests, isLoading } = useQuery({
    queryKey: ['leave-requests', statusFilter, employeeFilter, currentEmployee?.id],
    queryFn: () => {
      // For employees, force their own data only
      const employeeId = canViewAll 
        ? (employeeFilter && employeeFilter !== 'all' ? parseInt(employeeFilter) : undefined)
        : currentEmployee?.id;
  ...
  const rejectMutation = useMutation({
    mutationFn: ({ id, approved_by, rejection_reason }: { id: number; approved_by: number; rejection_reason: string }) =>
      backend.leave.reject({ leave_request_id: id, approved_by, rejection_reason }),
    onSuccess: () => {
      toast({
        title: "Thành công",
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (leaveBalance && (formData.leave_type === 'annual' || formData.leave_type === 'sick')) {
      let availableDays = 0;
      let leaveTypeName = '';
    ...
    const submitData = {
      ...
      end_date: endDate,
      reason: formData.reason && formData.reason.trim() !== '' ? formData.reason.trim() : undefined,
      employee_id: currentEmployee.id,
    };
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">
            {canViewAll ? 'Đơn xin nghỉ phép và phê duyệt' : 'Đơn xin nghỉ phép và theo dõi trạng thái'}
          </p>
        </div>
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="annual">
                        Nghỉ phép năm {leaveBalance && `(còn ${leaveBalance.balance.annual_leave_remaining} ngày)`}
                      </SelectItem>
                      <SelectItem value="sick">
                        Nghỉ ốm {leaveBalance && `(còn ${leaveBalance.balance.sick_leave_remaining} ngày)`}
                      </SelectItem>
          ...
          <CardContent>
            {balanceLoading ? (
              ...
              <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
                <p className="text-red-800">Lỗi khi tải thông tin nghỉ phép: {(balanceError as any)?.message || 'Unknown error'}</p>
                <p className="text-sm text-red-600 mt-2">Vui lòng thử lại sau hoặc liên hệ admin.</p>
              </div>
            ) : leaveBalance ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                  <div className="flex items-center justify-between mb-3">
                    <h4 className="text-sm font-medium text-blue-900">Nghỉ phép năm</h4>
                    <Clock className="h-4 w-4 text-blue-600" />
                  </div>
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      <span className="text-blue-700">Tổng cộng:</span>
                      <span className="font-medium text-blue-900">{leaveBalance.balance.annual_leave_total} ngày</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-blue-700">Đã sử dụng:</span>
                      <span className="font-medium text-blue-900">{leaveBalance.balance.annual_leave_used} ngày</span>
                    </div>
                    ...
                    <div className="w-full bg-blue-200 rounded-full h-2 mt-3">
                      <div 
                        ...
                        style={{ 
                          width: `${Math.min(100, (leaveBalance.balance.annual_leave_used / leaveBalance.balance.annual_leave_total) * 100)}%` 
                        }}
                      ></div>
                    </div>
                  </div>
                ...
                <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                  <div className="flex items-center justify-between mb-3">
                    <h4 className="text-sm font-medium text-green-900">Nghỉ ốm</h4>
                    <Clock className="h-4 w-4 text-green-600" />
                  </div>
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      <span className="text-green-700">Tổng cộng:</span>
                      <span className="font-medium text-green-900">{leaveBalance.balance.sick_leave_total} ngày</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-green-700">Đã sử dụng:</span>
                      <span className="font-medium text-green-900">{leaveBalance.balance.sick_leave_used} ngày</span>
                    </div>
                    <div className="border-t border-green-200 pt-2">
                      <div className="flex justify-between text-sm">
                        <span className="text-green-700 font-medium">Còn lại:</span>
                        <span className={`font-bold ${
                    ...
                    <div className="w-full bg-green-200 rounded-full h-2 mt-3">
                      <div 
                        ...
                        style={{ 
                          width: `${Math.min(100, (leaveBalance.balance.sick_leave_used / leaveBalance.balance.sick_leave_total) * 100)}%` 
                        }}
                      ></div>
                    </div>
                  </div>
            ...
            {/* Warning messages */}
            {leaveBalance && (leaveBalance.balance.annual_leave_remaining <= 2 || leaveBalance.balance.sick_leave_remaining <= 5) && (
                ...
                <div className="flex items-start">
                  ...
                  <div className="text-sm text-orange-800">
                    ...
                    <ul className="space-y-1">
                      {leaveBalance.balance.annual_leave_remaining <= 2 && (
                        <li>• Số ngày nghỉ phép năm còn lại ít ({leaveBalance.balance.annual_leave_remaining} ngày)</li>
                      )}
                      {leaveBalance.balance.sick_leave_remaining <= 5 && (
                        <li>• Số ngày nghỉ ốm còn lại ít ({leaveBalance.balance.sick_leave_remaining} ngày)</li>
                      )}
                    </ul>
                  </div>
                </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b">
                    ...
                    {canApprove && <th className="text-left py-3 px-4">Thao tác</th>}
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={request.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="py-3 px-4">
                        <div className="text-sm">
                          ...
                          <div className="text-gray-500">đến {formatDate(request.end_date)}</div>
                        </div>
                      </td>
                      <td className="py-3 px-4">
