      ...
      <div className="flex items-end justify-between h-full space-x-3">
        {data.map((item, index) => (
          <div key={item.date} className="flex flex-col items-center flex-1">
            <div className="flex flex-col items-center mb-2 h-full justify-end pb-8">
              {/* Attendance rate bar */}
              <div
                className="w-8 bg-blue-500 rounded-t relative group transition-all hover:bg-blue-600"
                style={{
                  height: `${(item.attendance_rate / maxRate) * 180}px`,
                  minHeight: "12px",
                }}
              >
              ...
              {/* Present count bar */}
              <div
                className="w-6 bg-green-400 rounded-t mt-1 relative group transition-all hover:bg-green-500"
                style={{
                  height: `${(item.present_count / maxPresent) * 120}px`,
                  minHeight: "8px",
                }}
              >
                <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-gray-800 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10">
